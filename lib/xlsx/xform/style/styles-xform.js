...
class StylesXform extends BaseXform {
  ...
  init() {
    ...
    this.weakMapCellStyles = new WeakMap();
  }
  ...
  render(xmlStream, model) {
    ...
    if (this.index) {
      ...
      if (!model.cellStyleXfs.length) {
        this._addCellStyle({numFmtId: 0, fontId: 0, fillId: 0, borderId: 0, xfId: 0});
      }
      xmlStream.openNode('cellStyleXfs', {count: model.cellStyleXfs.length});
      model.cellStyleXfs.forEach(cellStyleXml => {
        xmlStream.writeXml(cellStyleXml);
      });
      xmlStream.closeNode();
      ...
      this.map.cellStyleXfs.render(xmlStream, model.cellStyleXfs);
      this.map.cellXfs.render(xmlStream, model.styles);
    }

    if (!model.cellStyles.length) {
      model.cellStyles.push({name: 'Normal', xfId: 0, builtinId: 0});
    }

    this.map.cellStyles.render(xmlStream, model.cellStyles);
  ...
  // each style property is processed and cross-referenced, etc.
  // the styleId is returned. Note: cellType is used when numFmt not defined
  addCellStyleModel(model) {
    if (!model) {
      return 0;
    }

    // if we have seen this style object before, assume it has the same styleId
    if (this.weakMapCellStyles && this.weakMapCellStyles.has(model)) {
      return this.weakMapCellStyles.get(model);
    }
    ...
    const styleId = this._addCellStyle(style);
    if (this.weakMapCellStyles) {
      this.weakMapCellStyles.set(model, styleId);
    }
    return styleId;
  }

  addCellStyle(model) {
    this.model.cellStyles.push(model);
  }
  ...
  _addCellStyle(style) {
    const xml = this.map.style.toXml(style);
    let index = this.index.cellStyles[xml];
    if (index === undefined) {
      index = this.index.cellStyles[xml] = this.model.cellStyleXfs.length;
